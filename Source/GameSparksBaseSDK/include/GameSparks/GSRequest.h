	...
	namespace Core
	{
		...
		class GS_API GSRequest : public GSObject
		{
			public:
                ...
                {
					assert(!m_callbacks || m_callbacks->m_userData == m_userData); // user-data in member needs to be same as in m_callbacks
                    m_userData = userData;
					if (m_callbacks) // the callbacks member may also hold a user-data value
					{
						m_callbacks->m_userData = userData;
					}
                }
				...
				float getExpiresInSeconds() const
				{
					return m_expiresInSeconds;
				}
			private:
			...
			private:
				...
				bool m_Durable;
				Seconds m_expiresInSeconds = Seconds(-1);
				int m_durableAttempts = 1;
				...
				*/
				...
				void Send(Seconds timeoutSeconds);
