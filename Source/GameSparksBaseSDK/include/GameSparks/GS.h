	...
	namespace Core
	{
		...
		class GS_API GS // this naming should be discussed, it was chosen because we need the static object with the name "GS"
		{
			public:
				...
				#if defined(GS_USE_STD_FUNCTION)
				typedef gsstl::function<gsstl::string(const gsstl::string& nonce)> t_NonceCallback;
				#else
				typedef gsstl::string(*t_NonceCallback)(const gsstl::string& nonce);
				#endif /* GS_USE_STD_FUNCTION */

				/*!
                 *
				 * This is called after a nonce is received from the server. This callback is supposed to
				 * return the SHA256 HMAC based upon the nonce and the API-secret. This can be used to
				 * obfuscate the API secret or customize the computation of the hmac auth token in other
				 * ways (like calling a server to compute it, etc.).
				 *
				 * simple example:
				 *
				 * GS gs;
				 * MyPlatform platform(API_KEY, API_SECRET, ...);
				 * gs.OnNonce = [&](const gsstl::string& nonce)
				 * {
				 *   return GameSparks::Util::getHMAC(nonce, platform->GetGameSparksSecret());
				 * };
				 *
				 * */
				t_NonceCallback OnNonce;
			...
			private:
				...
				void ProcessPersistentQueue(Seconds deltaTimeInSeconds);
				...
				Seconds m_mustBeConnectedIn;
				Seconds m_sendNextDurableRequestIn;
